...
@property (nonatomic, copy) LoginFailture   loginFailture;

@property (nonatomic, copy) RegisterSuccess registerSuccess;
@property (nonatomic, copy) RegisterFailture   registerFailture;

@end

@implementation HLIMCenter

static HLIMCenter *_instance;
+ (instancetype)sharedInstance
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [HLIMCenter new];
    });
...
- (void)logout
{
    [[HLIMCenter sharedInstance].xmppStream disconnect];
    [[HLIMCenter sharedInstance].xmppStream removeDelegate:self];
    [HLIMCenter sharedInstance].loginFailture = nil;
    [HLIMCenter sharedInstance].loginSuccess = nil;
}
...
{
    self.registerFailture = failture;
...
- (void)xmppStreamDidConnect:(XMPPStream *)sender
{
    ...
    if (self.xmppNeedRegister) {
        NSError *error;
        [self.xmppStream registerWithPassword:self.myPassword error:&error];
...
- (void)xmppStreamDidDisconnect:(XMPPStream *)sender withError:(NSError *)error
{
    NSLog(@"%s",__func__);
    if (self.xmppNeedRegister) {
        if (self.registerFailture) {
            self.registerFailture(@{@"error":error});
        }
        return;
    }
    
    if (self.loginFailture && error) {
        
        NSDictionary *dict = @{@"error":error};
        self.loginFailture(dict);
    }
}
...
//    NSLog(@"%s--%@",__FUNCTION__, message);
    ...
    if ([dict objectForKey:@"roomId"]) {
        ...
        [WebRTCClient sharedInstance].roomId = dict[@"roomId"];
