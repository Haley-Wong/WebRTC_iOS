...
- (void)registerForRoomId:(NSString *)roomId
    ...
    if (_sinalingChannelState == kARDSignalingChannelStateOpen) {
        [self registerWithCollider];
    }
}

- (void)registerWithCollider {
    if (_sinalingChannelState == kARDSignalingChannelStateRegistered) {
        return;
    }
...
//    [self webSocketSendMessage:messageString];
    self.sinalingChannelState = kARDSignalingChannelStateRegistered;
}
...
- (void)webSocketSendMessage:(NSString *)message
{
    ...
    if (self.sinalingChannelState == kARDSignalingChannelStateRegistered) {
